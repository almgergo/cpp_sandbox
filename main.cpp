...
int main() {
    ...
    auto world = new World(0.001, 0.1);
    auto particles = std::vector<std::unique_ptr<Particle>>();
    particles.reserve(100);

    for (auto i = 0; i < N; i++) {
        auto position = std::array<double, 3>{dis(generator), dis(generator), dis(generator)};
        auto velocity = std::array<double, 3>{dis(generator)*0.01, dis(generator)*0.01, dis(generator)*0.01};
